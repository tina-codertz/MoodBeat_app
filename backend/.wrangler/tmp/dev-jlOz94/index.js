...
// .wrangler/tmp/bundle-RamH5v/strip-cf-connecting-ip-header.js
...
async function hashPassword(password) {
  ...
  const toHex = /* @__PURE__ */ __name((buf) => [...buf].map((b) => b.toString(16).padStart(2, "0")).join(""), "toHex");
...
app.use("/api/*", async (c, next) => {
  ...
  try {
    const payload = await verify2(authHeader.slice(7), c.env.JWT_SECRET, "HS256");
    c.set("userId", payload.sub);
    c.set("userEmail", payload.email);
    return next();
  } catch {
...
app.post("/api/auth/signin", async (c) => {
  ...
  const token = await sign2(
    { sub: user.id, email: user.email, exp: Math.floor(Date.now() / 1e3) + 60 * 60 * 24 * 7 },
    c.env.JWT_SECRET,
    "HS256"
  );
  return c.json({
...
app.get("/api/songs", async (c) => {
  ...
  if (tagsParam) {
    ...
    const query = `
      ...
      LIMIT 20
    `;
    const { results: results2 } = await c.env.DB.prepare(query).bind(...tags).all();
    const songs2 = (results2 || []).map(parseSongRow);
    return c.json({ songs: songs2 });
  }
...
__name(parseSongRow, "parseSongRow");
...
app.post("/api/playlists", async (c) => {
  ...
  ).bind(id, userId, name, description || "", cover_gradient || "").run();
  if (song_ids?.length) {
...
// .wrangler/tmp/bundle-RamH5v/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-RamH5v/middleware-loader.entry.ts
