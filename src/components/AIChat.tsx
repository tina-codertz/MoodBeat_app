...
function generateAIResponse(userMessage: string): string {
  ...
  if (
    msg.includes('nostalgic') ||
    msg.includes('memories') ||
    msg.includes('throwback') ||
    msg.includes('old')
  ) {
...
export function AIChat({
  ...
  const sendMessage = async (text: string) => {
    ...
    if (userId) {
      api
        .post('/api/chat/messages', { role: 'assistant', content: aiText })
        .catch(() => {});
    }

    const textLower = text.toLowerCase();
    const detectedMoodCard = MOOD_CARDS.find(
      (m) =>
