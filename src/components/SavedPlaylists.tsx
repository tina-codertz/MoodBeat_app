...
import { Loader2, ListMusic, Trash2, Play, ChevronRight, Music2 } from 'lucide-react';
...
export function SavedPlaylists({ userId, onAuthRequired }: SavedPlaylistsProps) {
  const { playlists, loading, fetchPlaylists, deletePlaylist } = usePlaylists(userId);
  ...
  const toggleExpand = async (playlistId: string) => {
    ...
    if (data) {
      setPlaylistSongs(prev => ({
        ...prev,
        [playlistId]: data.map((row: { songs: Song }) => row.songs).filter(Boolean),
      }));
    }
  ...
  if (!userId) {
    return (
      <div className="text-center py-20">
        ...
        <h3 className="text-white font-semibold mb-2">Sign in to save playlists</h3>
        <p className="text-white/40 text-sm mb-6">Your personalized playlists will appear here</p>
  ...
  if (playlists.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-white/40 text-sm">Select a mood on the Discover tab and save your playlist</p>
      </div>
    );
  }
    ...
    <div className="space-y-3 animate-slide-up">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-white/40 text-sm mt-0.5">{playlists.length} saved playlist{playlists.length !== 1 ? 's' : ''}</p>
        </div>
      </div>
      ...
      {playlists.map((playlist, index) => {
        ...
        return (
          ...
          >
            ...
            >
              {/* Cover */}
              <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${playlist.cover_gradient} flex items-center justify-center flex-shrink-0 relative overflow-hidden`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-white/40 text-xs mt-0.5 truncate">{playlist.description}</p>
            ...
            {isExpanded && (
              <div className="border-t border-white/5 p-3 animate-fade-in">
                {loadingSongs === playlist.id ? (
                  ...
                  <p className="text-white/30 text-sm text-center py-4">No tracks in this playlist</p>
                ) : (
                  <div className="space-y-1">
                    {songs.map((song, i) => (
                      <div key={song.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 transition-colors group">
                        ...
                        <span className="text-white/20 text-xs hidden sm:block">{song.genre}</span>
                      </div>
                    ))}
                  </div>
                )}
